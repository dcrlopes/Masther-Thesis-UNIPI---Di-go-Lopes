...
\begin{enumerate}
  \item A derived bound is never a substitute for a measured
        constraint. The upper reactivity bound of Campaigns 1 to 6 was
        replaced in Campaign 7 by a directly measured controllability
constraint, and the comparison showed the derived bound
        ...
        the historic bound of 1.35 its provenance, as the all-bank
...
Every design on the Campaign 8 front exceeds the reported refuelling
interval, the shortest by a third and the longest by a factor of four
(Section~\ref{sec:res-c8-mission}). The objective kept pushing the
cycle length long after the requirement was met, and it could do so
only by raising the beginning-of-life excess reactivity, which the
control rods then have to hold: between \num{8930} and
\SI{13627}{pcm} at power, with banks inserted deep into the core
throughout early life. A PWR does not operate
that way. Its regulating banks sit near the withdrawn position and
carry only the power defect and the manoeuvring band, while the excess
is held by boron adjustment and by burnable absorber whose burnout
tracks the fuel. The four Campaign 8 designs that behave that way still
deliver 1142 to \SI{3381}{EFPD} (Section~\ref{sec:res-c8-split}).
...
\begin{itemize}
  ...
  \item $g_j$ are the remaining constraints of
        ...
        enrichment limit, the vessel fit, the peaking constraint at 1.65, and
        the controllability constraint at beginning of life and at the operating
        maximum.
\end{itemize}
...
concentration exceeds the MTC boron limit of
...
problem at the five-year floor selects designs C8-47 and C8-13
...
Under the formulation of Campaigns 1 to 8, the candidate is design C8-47
...
\SI{2327}{ppm} is below the MTC boron limit at both primary conditions
(Sections~\ref{sec:res-c8-boron}
and~\ref{sec:res-c8-mtc}). Design C8-53 holds more margin and needs less
boron but meets five years only at a capacity factor of 0.8, and C8-31
loses its two-bank margin at the maximum of its hump.
...
Design C8-47 needs 419 to \SI{602}{ppm} more boron than
designs C9-35, C9-40 and C9-34, at a peaking factor that differs
by less than one and a half seed standard deviations, and C8-13 is
dominated by C9-47 and C9-44 on both objectives
...
Within Campaign 9 the best-ranked design is C9-47, the lowest boron demand of the
...
falls by 0.030 between the two models, and C9-44 leaves it, because
C9-47 then has both the lower peaking and the lower boron demand. The
first change was measured with six seeds, at five standard deviations,
and the second with two, at more than four
...
(Section~\ref{sec:res-c9-front}), and the MTC boron limit measured on the
...
uncertainty (Section~\ref{sec:res-c9-post}). Until the order of the front on the
peaking axis is resolved (Section~\ref{sec:res-c9-peaking}), design C8-47 is the most completely confirmed
...
\begin{enumerate}
  ...
  \item The controllability constraint tests hold-down at power with the operating
  ...
  \item Up to Campaign 8 the reactivity swing over the cycle was not
        ...
        maximum and applies the controllability constraint there, which closes this
  ...
  \item The statistical error on the peaking objective was not
        measured during the campaigns, only bounded. Measured a posteriori,
        the single-solve standard deviation is 0.011 on the Campaign 8
        designs at the confirmation statistics and 0.012 on the
        Campaign 9 front at the campaign statistics. At the campaign
        statistics the core estimator also carries an upward bias of
        $+0.060$ to $+0.077$, which varies by 0.017 between front
        designs. Five of the ten pairs of the Campaign 9 front are not
        resolved by one campaign solve, and at the extrapolated values
        C9-35 leaves the front. The peaking objective is also two-dimensional,
        and on the Campaign 9 front the three-dimensional value changes
        which designs are non-dominated.

  \item Every design below a pitch of \SI{1.224}{cm} in Campaigns 1
  ...
  \item Thermal-hydraulic feedback is absent. Temperatures are fixed at
        ...
        Campaign 9 lattices, and enters only through the MTC boron limit.

  \item The MTC boron limit was measured on the lattice of
        C8-47 and on eight Campaign 9 lattices at \SI{12.8}{MPa},
        five of them also at \SI{15.5}{MPa}. It rises with the
        gadolinia loading, but the eight lattices confound that loading
        with the reflector thickness, which is at its bound for seven of
        them. The fitted relation therefore holds only inside the
        converged region of Campaign 9, and the limit of any other
        lattice has to be measured.

  \item The feasibility margin of the acquisition cannot act on a
        constraint that is active at the optimum. In Campaign 9 the
        cycle-length floor is such a constraint, no candidate passed the
        margin, and every infill batch was ordered by the fallback. The
        design of experiments of every campaign was also sampled without
        a fixed seed, so a campaign is reproducible from its archive
        and not from its launch settings.
\end{enumerate}
...
Campaign 9 complete except for the order of its front on the peaking
axis. Table~\ref{tab:concl-future} lists the
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{3.6cm}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}p{4.0cm}@{}}
    ...
    \midrule
    Peaking order of the Campaign 9 front
      & Eight seeds per front design at $400\,000$ particles, or a bias correction of the campaign setting from the core-level fidelity study
      & The final candidate, since five of the ten front pairs are not resolved on the peaking axis \\
    \addlinespace
    Shutdown margin
    ...
    Core-level depletion
      & Depletion of the core in place of the reflected assembly, at the cost of one core depletion per evaluation
    ...
    \addlinespace
    Margin at an active constraint
      & Either constrain the surrogate search itself on $\mu_{ij} + \kappa\,\sigma_{ij} \le 0$, the confidence-bound rule of safe exploration applied inside the search~\cite{sui_safeopt_2015}, at the cost of a front held about \SI{42}{d} above the floor, or weight the uncertainty score by the probability of feasibility of constrained Bayesian optimisation~\cite[Sec.~5.4.2]{forrester_book_2008}, which keeps a boundary design at half weight instead of excluding it
      & An acquisition that still discriminates when the optimum is located on a constraint, as in Campaign 9 \\
    \addlinespace
    Seeded design of experiments
      & Pass the campaign seed to the Latin-hypercube sampler
      & A campaign reproducible from its launch settings as well as from its archive \\
    \addlinespace
...
formulation. What remains beyond the limits, the three-dimensional power
