...
%%
%%   \begin{appendices}
%%   \input{appendix_model_inputs}                            (Appendix A)
%%   \input{appendix_code_architecture}                       (Appendix B)
%%   \end{appendices}
%%
...
\texttt{main}, which carries the merged state of every campaign branch and
is the only branch developed after Campaign~8. The state described in this
appendix is commit \texttt{e4a3582} of 8 September 2026.

The campaign branches \texttt{campaign4} to \texttt{campaign8} are kept in
the repository as a record of the state under which each campaign was run.
They receive no further commits. The files that define the geometry, the
materials, the operating point and the three-dimensional structural components are
byte-identical between \texttt{campaign8} and \texttt{main}, so citing
\texttt{main} changes no input reported in
Appendix~\ref{app:model-inputs}. The two files that do differ,
\file{openmc_evaluator.py} and \file{run_optimization.py}, differ in
comment text and in three command-line flags added for the validation
campaign, and in no physical or numerical constant.

\openpoint{Update the commit hash above to the final state of branch
\texttt{main} at submission. In the working clone the command is
\file{git rev-parse --short HEAD}.}

The repository contains 117 Python scripts and 12 shell wrappers. Only the
...
\cite{romano_openmc_2015,romano_2021_depletion}, the Monte Carlo transport and depletion code.
The coral block, the surrogate-assisted optimiser, is the only one that
never calls OpenMC directly. Grey blocks are infrastructure and outputs.
Unlabelled arrows are Python imports. Labelled arrows name the file, or
the pair of vectors, exchanged. The scripts listed inside each block are
...
\begin{figure}[htbp]
  ...
  \caption{Macro architecture of the simulation code.}
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{3.6cm} X@{}}
    ...
    \file{zoning.py} & Ring map, centre, middle and periphery multipliers,
      ...
      history reader \\
    \file{hardware3d.py} & Finite-height core with the axial stack of assembly
      structural components, used by the three-dimensional confirmation \\
    \bottomrule
  \end{tabularx}
\end{table}

\subsection{Leakage-corrected reactivity target}
...
dependence of the leakage. Table~\ref{tab:arch-ktarget} lists its scripts.
The two validation scripts read the campaign checkpoint to select the
designs they evaluate.

\begin{table}[htbp]
  \centering\footnotesize
  \caption{Reactivity-target block.}
  \label{tab:arch-ktarget}
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{5.0cm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}p{4.4cm}@{}}
    \toprule
...
Table~\ref{tab:arch-ctrl} lists its scripts. All of them read the campaign
checkpoint except \texttt{derive\_kmax.py}, which reads the screen written
by \texttt{rod\_bank\_worth.py}.

\begin{table}[htbp]
  \centering\footnotesize
  \caption{Control-rod block.}
  \label{tab:arch-ctrl}
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{5.0cm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}p{4.4cm}@{}}
    \toprule
    Script & Role & Main output \\
...
Table~\ref{tab:arch-optimiser} lists its scripts. The driver reads the
target table and, on resume, the checkpoint, and the two analysis scripts
read the checkpoint.

\begin{table}[htbp]
  \centering\footnotesize
  \caption{Optimiser block.}
...
Table~\ref{tab:arch-confirm} lists them.

\begin{table}[htbp]
  \centering\footnotesize
  \caption{Confirmation block.}
  \label{tab:arch-confirm}
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{5.0cm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}p{4.4cm}@{}}
    \toprule
    Script & Role & Main output \\
    \midrule
    \file{confirm3d.py} & Three-dimensional confirmation with the structural
      components, several rod states and boron concentrations & \file{summary.json} \\
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}p{5.1cm} >{\raggedright\arraybackslash}p{2.1cm} >{\raggedright\arraybackslash}p{2.5cm} >{\raggedright\arraybackslash}X@{}}
    \toprule
