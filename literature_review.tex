...
\begin{itemize}
  ...
  \item The Chinese ACP100, also called Linglong One, is an integrated PWR
...
The IAEA projects global nuclear capacity each year. Against an
...
Deployment lags the projections. The IAEA documents more than 80 SMR
and microreactor designs, and only two have entered commercial
...
goals~\cite{marinha_relatorio_gestao_2025}:
...
LABGENE-class model, and Figure~\ref{fig:lr-maruyama-vessel} reproduces
the simplified vessel drawing the envelope follows, prepared for a
structural analysis according to the Centro Tecnológico da Marinha em
São Paulo (CTMSP)~\cite{maruyama_2013}. The dimensions it gives are
listed later in this section.

The internals, the coolant path and the control rods in the left panel
are illustrative and are not to scale. The coolant enters at the inlet
...
\begin{figure}[htbp]
  ...
  \begin{subfigure}[b]{0.54\textwidth}
    \centering
    \includegraphics[width=\linewidth]{images/th_vessel_components}
    \caption{Components of the vessel}
    \label{fig:vessel-components}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.42\textwidth}
    \centering
    \includegraphics[width=\linewidth]{images/lr_maruyama_vessel.pdf}
    \caption{Dimensions adopted according to CTMSP}
    \label{fig:lr-maruyama-vessel}
  \end{subfigure}
  \caption[LABGENE-class reactor vessel: components and adopted dimensions]{LABGENE-class reactor vessel: components and adopted dimensions.\newline Vessel envelope and dimensions adapted from~\cite{maruyama_2013}.}
  \label{fig:lr-vessel}
\end{figure}
...
\begin{itemize}
  ...
  \item Its fuel is low-enriched uranium, as declared by Brazil at the
        ...
        Indústrias Nucleares do Brasil (INB) produced the pellets of
  ...
  \item The submarine is named Álvaro Alberto and is part of the
...
\begin{itemize}
  ...
  \item Kassenova reported 18 to 19~wt\%~\cite{kassenova_2014}, a figure
        given by a former chairman of the Comissão Nacional de Energia
...
CTMSP, reproduced in Figure~\ref{fig:lr-maruyama-vessel}. It gives the
following dimensions~\cite{maruyama_2013}:
...
each method, to maximise the cycle length while keeping
