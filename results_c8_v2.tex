...
\begin{table}[htbp]
  \centering
  \small
  \caption{The Campaign 8 front. Ring enrichments are the as-built
  centre, middle and periphery values. Excess is the beginning-of-life
  reactivity at \SI{1000}{ppm} that the rods must hold. $F_{\Delta H}$
  is given with all rods out, with two banks in and with four banks
  in.}
  \label{tab:c8-front}
  \begin{tabular}{ccccccccccc}
    \toprule
    Design & EFPD & rings [wt\%] & $w_\mathrm{Gd}$ & $n_\mathrm{Gd}$ & $t_\mathrm{refl}$ & $k^\mathrm{core}$ & excess [pcm] & $F$ & $F_{2}$ & $F_{16}$ \\
    \midrule
...
\begin{table}[htbp]
  \centering
  \small
  \caption{The feasible designs controllable with the first two banks.
  Design 12 is included for completeness and is not a candidate.}
  \label{tab:c8-two}
  \begin{tabular}{cccccccccc}
    \toprule
    Design & EFPD & rings [wt\%] & $w_\mathrm{Gd}$ & $n_\mathrm{Gd}$ & $t_\mathrm{refl}$ & $k^\mathrm{core}$ & excess [pcm] & $F$ & $F_{2}$ \\
    \midrule
...
\begin{table}[htbp]
  \centering
  \small
  \caption{Beginning-of-life reactivity to be held by the rods, with
  ...
  column is for sixteen control rod assemblies in every case: the
  benchmark's four banks together, and the four regulating banks of
  this core.}
  \label{tab:bench-compare}
  \begin{tabular}{lccc}
    \toprule
    Core & $k_\mathrm{eff}^\mathrm{core}$ & excess to hold [pcm] & worth of sixteen assemblies [pcm] \\
    \midrule
    NuScale-like benchmark~\cite{fridman_nuscale_benchmark_2023} & 1.028 & 2700 & 19231 \\
    Campaign 8 front, Table~\ref{tab:c8-front}                   & 1.098 to 1.158 & 8930 to 13627 & 14270 to 17665 \\
    Campaign 8 two-bank subset, Table~\ref{tab:c8-two}           & 1.026 to 1.042 & 2564 to 4068 & 13906 to 16303 \\
    \bottomrule
  \end{tabular}
\end{table}
...
every phase, so the breakdown is exact: assembly solve \SI{11}{s}, core
solve \SI{107}{s}, depletion \SI{925}{s}, and the two rodded solves
\num{100} and \SI{102}{s}. The rodded solves are \SI{16.2}{\%} of the
cost. The depletion dominates because the cycles are long, 14.6
...
\begin{enumerate}
  ...
  \item The convergence claim is one block short of the formal rule.
\end{enumerate}
