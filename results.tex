...
\begin{figure}[htbp]
  ...
  gadolinium burnout maximum is visible between \num{7.5} and
...
Figure~\ref{fig:c8-swing-khist} shows the ten histories with their
maxima marked and Table~\ref{tab:c8-swing} reports the quantities.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\linewidth]{images/swing_khist}
  \caption[Depletion histories of the Campaign 8 candidates]{Assembly
  infinite multiplication factor against burnup at \SI{1000}{ppm} for
  the eight front designs and the two two-bank representatives.
  Triangles mark the operating maximum and the dotted line is the
...
\begin{table}[htbp]
  ...
  \end{tabular}
  \begin{tablenotes}\footnotesize
    \item Assembly $k_\infty$ at 1000\,ppm from the single-shot depletion of \texttt{validate\_ktarget\_burnup.py} at the campaign fidelity. The hump is $\rho_\mathrm{peak} - \rho_\mathrm{BOL}$, xenon-free BOL. $\Delta\rho$ is measured to $\rho(k_\mathrm{target})$. The boron concentrations are linear extrapolations above 1000\,ppm with the 1000 to 1500\,ppm differential worth of the core and are lower bounds.
  \end{tablenotes}
  \end{threeparttable}
\end{table}
...
\begin{enumerate}
  \item Half of the candidates have no hump. Designs 1, 23, 42, 47 and
        53 reach their maximum at the first operating point, and their
        hump is negative, from $-626$ to \SI{-1591}{pcm}. For these
        designs the beginning-of-life state is the most reactive state
        of the cycle and the screens of Section~\ref{sec:meth-ctrl}
        screen the right state.

  \item The other half do have one, from $+1150$ to \SI{+4580}{pcm},
  ...
  \item The campaign was screened at a boron concentration well below
        ...
        44. Every value is a lower bound, because the extrapolation is
        linear above \SI{1500}{ppm} and ignores further
        self-shielding.
\end{enumerate}
...
%% AUTHOR: the physics of why five candidates show no hump while five
...
three-dimensional confirmation on the core with hardware. This
...
\begin{enumerate}
  ...
  \item The three departures from the benchmark inputs stated in Section~\ref{sec:reactor-model}. \openpoint{Closure 3a: reflector sensitivity pending. } \openpoint{Closure 3b: absorber-radius sensitivity pending. } The coolant density is not exposed as a sensitivity and remains a stated departure.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}l l r l r@{}}
    ...
    3a Reflector 0.956 & $\Delta\rho$ unrodded 3D [pcm] & pending &  &  \\
    3b Absorber 0.4229 & $\Delta$ four-bank margin [pcm] & pending & $\Delta$ two-bank margin [pcm] & pending \\
...
need to reject while admitting none that the hardware would refuse.
...
Section~\ref{sec:res-c8-closures}, with the hardware stack, the core
barrel and the boron worth, decides between them.
...
\begin{enumerate}
  ...
  \item A zero-boron hold-down check under the four regulating banks on
        the two-dimensional core and on the three-dimensional core with
        the assembly hardware, barrel and vessel, two seeds (44 solves).

  \item A three-dimensional confirmation at 1000\,ppm in the three rod
        states, two seeds, for ten of the eleven designs. Design 13 was
        not run.
...
Section~\ref{sec:meth-ctrl}.
Figure~\ref{fig:c8-post-banks} shows the map.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{images/c8_post_core_banks_rings}
  \caption{The 32-assembly map with the bank assignment used by every
  screen of Campaign 8. (a) The four regulating banks and the shutdown
  bank. (b) The two-bank state RE1 + RE2. (c) The three enrichment rings
  with their multipliers. Ring C coincides with RE1, ring M with RE2 to
  RE4 and ring P with SH.}
...
criticality.
%% AUTHOR: if a licensing basis is adopted for this reserve, cite it
%% here. Otherwise keep it stated as an engineering choice of this work,
%% which is what the sentence above does.
...
\begin{figure}[htbp]
  ...
  \caption{The Campaign 8 archive in the objective plane. Blue circles
  are feasible designs that need the four regulating banks at 1000\,ppm.
  Green squares are feasible designs controllable with RE1 + RE2. The
  two lines are the two fronts. Error bars are one seed-to-seed standard
  deviation of $F_{\Delta H}$ at the archive fidelity, about 0.011. The
  dotted lines mark four and five calendar years at 48\,MWth for an
  assumed capacity factor of 0.8.}
...
core and $+1330$\,pcm on the three-dimensional core with hardware, and
that of design 42 goes from $-1315$ to $+1052$\,pcm.
Table~\ref{tab:c8-post-3d1000} and Figure~\ref{fig:c8-post-margins1000}
show the ten confirmed designs. Under the three-dimensional reading the
two-bank set at 1000\,ppm and beginning of life is 53, 31, 47 and 42,
design 23 stays out at $-1718$\,pcm, and design 13 is unconfirmed. That
...
\begin{figure}[htbp]
  ...
  \caption{Hold-down margins at 1000\,ppm and beginning of life for the
  ten confirmed designs, two seeds per solve. Dark bars are the
  two-dimensional core of the campaign screen, light bars are the
  three-dimensional core with hardware, barrel and vessel wall. Blue is
  the four-bank state and green the two-bank state. The dashed line is
  the screen of Equation~\eqref{eq:g-ctrl-meth}, $k \leq 0.99$, which is
  1010\,pcm on this axis.}
...
concentration. Three regularities stand out.
...
\begin{figure}[htbp]
  ...
  \caption{Beginning-of-life hold-down margin against soluble boron for
  the eleven designs, two-dimensional core, one seed. Left, the four
  regulating banks. Right, RE1 + RE2. Solid lines are front members and
  dashed lines are the retained two-bank designs. The dashed horizontal
  line is the screen of Equation~\eqref{eq:g-ctrl-meth}, $k \leq 0.99$, which
  is 1010\,pcm on this axis.}
...
Figure~\ref{fig:c8-post-boron} shows it. The three tier B designs reach
...
\begin{figure}[htbp]
  ...
  \caption{(a) Soluble boron needed at beginning of life for a
  hold-down margin equal to the screen of Equation~\eqref{eq:g-ctrl-meth}
  with sixteen and with eight rodded assemblies, eleven designs. Values marked extrap. lie beyond the
  1500\,ppm sweep point. (b) Four-bank and two-bank worth at 1000\,ppm
  across the 60 archive designs. (c) Differential boron worth of the
  eleven designs.}
...
Figure~\ref{fig:c8-post-zero} give the result. The condition tested is
the following.
...
\begin{figure}[htbp]
  ...
  \caption{Zero-boron hold-down margin under the four regulating banks
  at beginning of life. Grey, the one-seed sweep. Blue, the two-seed
  two-dimensional solve of stage D. Orange, the three-dimensional core
  with the assembly hardware, barrel and vessel wall. Green markers meet
  the screen $k \leq 0.99$, orange markers are subcritical below it, red
  markers are supercritical.}
...
Design 47 is the only member of the feasible front that satisfies
Equation~\eqref{eq:boronfree-necessary}. It is the boron-bearing
candidate of tier A and, at the same time, the design on which a
boron-free variant of the core can be studied. Equation~\eqref{eq:boronfree-necessary}
is a necessary condition, not a demonstration. Three things it does not
cover are listed in Section~\ref{sec:res-c8-post-limits}.
...
Figure~\ref{fig:c8-post-nesting} shows the structure.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{images/c8_post_constraint_structure}
  \caption{The rodded eigenvalues against the unrodded core eigenvalue
  for the 60 archive designs at 1000\,ppm. (a) The four-bank state, with
  the twelve designs rejected by the control screen alone circled in
  red. (b) The two-bank state. The dashed line is the screen value
  0.99, the dotted lines are $k_{\min}$ and $k_{\max}$.}
...
core. With all rods out the three-dimensional core with hardware gives
$L_\mathrm{ax} = k_\mathrm{2D} / k_\mathrm{3D}$ between 1.0286 and
1.0311 at 1000\,ppm over the ten confirmed designs, against the 1.0282
...
Table~\ref{tab:c8-post-3d1000} over the ten confirmed designs. The two-dimensional eigenvalues of
the confirmation reproduce the archive within $-53$ to $+69$\,pcm over
the thirty comparisons.

The unrodded axial factor splits by enrichment. The five designs at or
below \SI{5.90}{wt\%} give 1.0302 to 1.0311 and the five at or above
\SI{6.59}{wt\%} give 1.0286 to 1.0291. The campaign value 1.0289 is
therefore right for the long-cycle designs and 130 to 220\,pcm low for
...
%% Stage E on design 47 isolates the first at 132 pcm, so the second
...
difference of 132\,pcm, because the withdrawn rod leaves its steel end
...
Table~\ref{tab:c8-post-downcomer} gives the values. A downcomer of
...
0.3 to 1.7\,cm. Figure~\ref{fig:c8-post-refl} shows both.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{images/c8_post_reflector_leverage}
  \caption{Peaking factor and unrodded core eigenvalue against
  reflector thickness for the 60 archive designs, coloured by
  enrichment. Circles are feasible designs. The red arrows are the
  stage G solves with the reflector thinned to 3.969\,cm.}
...
\begin{enumerate}
  \item Every margin is a beginning-of-life value on fresh, xenon-free
        ...
        \openpoint{Run c8\_khist\_hump.py on the eleven designs. If the
        mid-cycle maximum of $k_\infty$ exceeds the xenon-free
        beginning-of-life value, subtract the difference from every margin
        of Sections~\ref{sec:res-c8-boron} and~\ref{sec:res-c8-zeroboron}
        before the candidate decision.}
  ...
  \item Design 13 carries no three-dimensional confirmation at
        1000\,ppm. Its two-bank status rests on the two-dimensional
        screen, which the confirmation of the other ten designs shows to
        be conservative by \num{2062} to \SI{2671}{pcm} in that state, so
        the status is expected to hold, but it is not measured.
        \openpoint{Run confirm3d.py on design 13 in the three rod states
        with two seeds, twelve solves, about fifty minutes, into
        confirm3d\_c8.}
...
%% AUTHOR: candidate decision. The post-analysis supports the following
%% reading, to be confirmed by the hump check. Design 47 satisfies all
%% three readings at once: it is the low-peaking floor of the feasible
%% front (1941 EFPD, F 1.510, 4.39 wt%), it is two-bank controllable at
%% 1000 ppm on the three-dimensional core (+1330 pcm), and it is the only
%% front member that satisfies eq:boronfree-necessary (+3069 pcm under
%% the four regulating banks at 0 ppm). Its 1941 EFPD are 6.6 calendar
%% years at a capacity factor of 0.8, above the four-to-five-year
%% interval reported for the prototype, so the short cycle is a cost
%% relative to the rest of the front, not a shortfall against the
%% reference envelope. The long-cycle alternative with boron is design 21
%% at the knee (4221 EFPD, F 1.592, 7.81 wt%), four-bank only, not
%% boron-free. The two-bank long-cycle alternative is design 31 (3381
%% EFPD, F 1.650, 6.59 wt%, at the enrichment reported in 2014), holding
%% 5856 pcm with two banks in 3D and 8684 pcm boron-free. Write the
%% choice and the physical argument here and in conclusions.tex.
