...
gadolinia rod count. The exercise contrasts 35 enumeration evaluations
with 20 search evaluations at the same transport fidelity.

\openpoint{Insert \texttt{valgrid\_table.tex}, written by
\texttt{valgrid\_compare.py}, once \texttt{run\_valgrid.sh} has
finished, and report the hypervolume ratio of
Equation~\eqref{eq:val-hvratio}, the inverted generational distance of
Equation~\eqref{eq:val-igd} and the additive epsilon indicator of
Equation~\eqref{eq:val-eps} against the enumerated front, together with
the evaluation count of each run.}

%% AUTHOR: the physical reading of the slice, namely which constraint
%% bounds the enumerated front on each side, is yours to state.
